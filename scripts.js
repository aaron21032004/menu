...
function displaySpecialItems() {
    const specialsSection = document.getElementById('special-items');
    const specials = menuItems.filter(item => item.special);
    specialsSection.innerHTML = specials.map(item => `
        <div class="menu-item">
            <img src="${item.image}" alt="${item.name}">
            <h3>${item.name}</h3>
            <p>${item.description}</p>
...
function placeOrder(event, itemId) {
    ...
    alert(`You have ordered ${quantity} x ${item.name} for a total of ${item.price}`);
}

function initializeMenu() {
    ['breakfast', 'lunch', 'snacks', 'beverages'].forEach(displayMenuItems);
    displaySpecialItems();
}
